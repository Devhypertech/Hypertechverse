      ...
      <div className="bg-[#1d1d1d]/30 backdrop-blur-xl shadow-[0_2px_24px_rgba(0,0,0,0.35)]">
        <div className="mx-auto flex h-24 max-w-full items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <Link href="/" className="flex items-center gap-2">
            <span className="text-6xl font-extrabold leading-none text-[#fff9f1]">HTV</span>
            <span className="hidden text-[#fff9f1]/90 font-semibold sm:block">
              Hypertech <br /> <span className="text-[#fff9f1]">Verse</span>
            </span>
          </Link>
                      ...
                      {n.dropdown ? (
                        <div className="relative group">
                          <div className="flex items-center">
                            <Link
                              href={n.href}
                              className={`text-base font-extrabold tracking-tight transition-all duration-400 ${active ? "text-[#fff9f1]" : "text-[#fff9f1]/90 group-hover:text-[#1d1d1d]"
                                }`}
                            >
                              <span style={{
                                fontSize: "24px",
                            ...
                            <svg
                              className="ml-1 w-4 h-4 text-[#fff9f1]/90 group-hover:text-[#1d1d1d] transition-all duration-200 group-hover:rotate-180"
                              fill="none"
                              stroke="currentColor"
                              viewBox="0 0 24 24"
                            >
                          ...
                          <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 w-screen max-w-7xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 ease-in-out transform translate-y-2 group-hover:translate-y-0 z-50 pointer-events-none group-hover:pointer-events-auto">
                            <div className="bg-[#fff9f1] shadow-xl border-t border-gray-100 overflow-hidden rounded-xl">
                                ...
                                <div className="grid grid-cols-6 gap-6">
                                  {n.dropdown.map((item: any, index: number) => (
                                    <Link
                                      key={item.href}
                                      href={item.href}
                                      className="group/item flex flex-col items-center text-center p-4 rounded-lg  transition-all duration-400 hover:scale-140"
                                    >
                                      {/* Icon Circle */}
                                      <div className={`w-16 h-16 rounded-full ${item.iconBg || 'bg-[#EA7BBF]'} flex items-center justify-center mb-3 group-hover/item:scale-110 transition-transform duration-400`}>
                                        {item.icon ? (
                                          <Image
                                            src={item.icon}
                                            alt={item.label}
                                          ...
                                          <div className="w-8 h-8 bg-[#fff9f1] rounded-full"></div>
                                        )}
                                      </div>
                                      {/* Title */}
                        ...
                        <Link
                          href={n.href}
                          className={`text-base font-extrabold tracking-tight transition-all duration-300 ${active ? "text-[#fff9f1]" : "text-[#fff9f1]/90 hover:text-[#1d1d1d]"
                            }`}
                        >
                          <span style={{
                            fontSize: "24px",
          ...
          <button
            aria-label="Menu"
            onClick={() => setOpen((s) => !s)}
            className="md:hidden inline-flex h-9 w-9 items-center justify-center rounded-md border border-white/20 text-[#fff9f1]"
          >
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <Link
                            ...
                            className={`block rounded-md px-1 py-1 text-base font-semibold transition ${active ? "text-[#fff9f1]" : "text-[#fff9f1]/90 hover:text-[#fff9f1]"
                              }`}
                          >
                            {n.label}
                          </Link>
                          <button
                            ...
                            className="p-1 hover:bg-[#fff9f1]/10 rounded transition-colors duration-200"
                          >
                            <svg
                        ...
                        {servicesOpen && (
                          <div className="ml-4 mt-2 bg-[#fff9f1] rounded-lg shadow-lg p-4">
